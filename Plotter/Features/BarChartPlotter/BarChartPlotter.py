...
def main():
    ...
    for i, (city, temperatures) in enumerate(cities_data.items()):
        offset = (i - (n - 1) / 2) * bar_width / n  # 中央に配置
        ax.bar([p + offset for p in x], temperatures, width=bar_width / n, label=city)
    ...
    # グラフ出力
    plt.savefig("./BarChartPlotter.pdf", bbox_inches="tight", pad_inches=0.05)
    plt.show()
