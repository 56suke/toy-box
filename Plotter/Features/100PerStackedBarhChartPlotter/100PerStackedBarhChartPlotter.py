...
###
# /* 100PerStackedBarhChartPlotter.py */
# 
# 
###
...
def main():
    ...
    # データを100パーセント積立棒グラフ用に正規化
    data_sums = np.sum([list(data) for data in zip(*cities_data.values())], axis=1)  # 各年月ごとの合計
    normalized_data = {city: np.array(data) / data_sums * 100 for city, data in cities_data.items()}

    # 系列数を計算
    bar_height = 0.8
    y = range(len(date))

    # 各都市のデータを積み上げ横棒グラフとしてプロット
    left = np.zeros(len(date))
    for city, data in normalized_data.items():
    ...
    # グラフ出力
    plt.savefig("./100PerStackedBarhChartPlotter.pdf", bbox_inches="tight", pad_inches=0.05)
    plt.show()
